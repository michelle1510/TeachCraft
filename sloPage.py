...
class sloPage(tk.Frame):
    ...
    def create_widgets(self):
        ...
        # Create a label
        self.label = tk.Label(self, text="Enter SLO:")
    ...
    def create_table(self):
        ...
        cursor.execute('''CREATE TABLE IF NOT EXISTS slos (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, goal INTEGER)''')
    ...
    def save_input_data(self, name):
        ...
        goal = None #goal is empty, to be updated later

        # Insert data into the table using parameterized query
        cursor.execute("INSERT INTO slos (name, goal) VALUES (?, ?)", (name, goal))
    ...
    def populate_previous_inputs(self):
        ...
        cursor.execute("SELECT name FROM slos")
    ...
    def clear_text(self):
        ...
        conn = sqlite3.connect('table.db')
        cursor = conn.cursor()
        cursor.execute("DELETE FROM slos")
        conn.commit()
        conn.close()
...
if __name__ == "__main__":
    ...
    multi_input_frame = sloPage(root)
    multi_input_frame.pack(side="left", padx=10, pady=10)

    # Create the table if it doesn't exist
    multi_input_frame.create_table()
