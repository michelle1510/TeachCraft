...
class proPage(tk.Frame):
  ...
  def create_widgets(self):
      ...
      # Create a label
      self.label = tk.Label(self, text="Enter PRO:")
  ...
  def create_table(self):
      ...
      cursor.execute('''CREATE TABLE IF NOT EXISTS pros (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, goal INTEGER)''')
  ...
  def save_input_data(self, name):
      ...
      cursor.execute("INSERT INTO pros (name, goal) VALUES (?, ?)", (name, goal))
  ...
  def populate_previous_inputs(self):
      ...
      cursor.execute("SELECT name FROM pros")
  ...
  def clear_text(self):
      ...
      conn = sqlite3.connect('TeachCraft/table.db')
      cursor = conn.cursor()
      cursor.execute("DELETE FROM pros")
      conn.commit()
      conn.close()
...
if __name__ == "__main__":
  ...
  multi_input_frame = proPage(root)
